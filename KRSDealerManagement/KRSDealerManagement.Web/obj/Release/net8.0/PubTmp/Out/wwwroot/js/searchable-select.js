(function () {
    ...
    function shouldEnhance(select) {
        ...
        if (select.closest('.grid-column-filters')) return false;
        return true;
    }
    ...
    function buildConfig(select) {
        ...
        var modalParent = select.closest('.modal-content') || select.closest('.modal');
        return {
            ...
            hideSelected: false,
            dropdownParent: modalParent ? modalParent : 'body',
            create: false,
            placeholder: isMultiple ? 'Select one or more...' : undefined,
            sortField: { field: 'text', direction: 'asc' },
            onDropdownOpen: function (dropdown) {
                if (this.input && this.input.closest('.grid-column-filters')) return;
                var controlWidth = this.wrapper ? this.wrapper.getBoundingClientRect().width : 0;
                var minWidth = Math.max(controlWidth, 240);
                dropdown.style.width = 'auto';
                dropdown.style.minWidth = minWidth + 'px';
                dropdown.style.maxWidth = 'min(36rem, 95vw)';
            },
